...
export default function SignupPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const body: any = { password, displayName };
  ...
  return (
    <main className="mx-auto max-w-md p-6">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          <label className="block text-sm text-steam-gold-300">メールアドレス（任意）</label>
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded border border-steam-iron-700 bg-steam-iron-900 p-2" />
        </div>
        <div>
          <label className="block text-sm text-steam-gold-300">ユーザーID（任意、英数_ 3-30文字）</label>
        ...
        <button disabled={loading} className="rounded bg-steam-gold-500 px-4 py-2 text-black hover:bg-steam-gold-400 disabled:opacity-60">{loading ? '作成中…' : '作成'}</button>
      </form>
    </main>
  );
}
